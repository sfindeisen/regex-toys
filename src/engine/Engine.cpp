...
std::vector<TSubCFAInitRes> Engine::initCFA_seq(CFA& cfa, const TCounterIdx& parentCounter, const AbstractRegexSequence* rx) const {
...
TSubCFAInitRes Engine::initCFA_simple(CFA& cfa, const TCounterIdx& parentCounter, const AbstractRegex* rx) const {
    if (nullptr == rx)
...
std::tuple<bool, unsigned int, unsigned int> Engine::computeCounter(const AbstractRegex& rx) const {
    if (! rx.hasMulti())
        RX_THROW(IllegalArgumentException);
...
TSubCFAInitRes Engine::initCFA_wrapWithCounter(CFA& cfa, const TCounterIdx& counterIdx, const AbstractRegex *rx) const {
...
TSubCFAInitRes Engine::initCFA(CFA& cfa, const TCounterIdx& parentCounter, const AbstractRegex *rx) const {
    if (nullptr == rx)
        RX_THROW(IllegalArgumentException);
